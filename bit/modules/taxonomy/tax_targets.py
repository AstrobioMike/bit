"""
The `-t` TARGET vocabulary shared by every surface that takes a taxon.

`-t` accepts a taxon name (see tax_ranks.TAXON_ALIASES for the spellings that get
...
explicitly by themselves (e.g., `-t Uroviricota` works fine)
"""

from bit.modules.taxonomy.tax_ranks import RANKS
from bit.modules.taxonomy.tax_counts import (count_genomes, count_distinct_taxa,
                                               distinct_taxa)
...
class UnassignedDomainSummary:
    ...
    def message(self, source_label="NCBI"):
        ...
        line = (f"{self.n_genomes:,} genome(s) in the {source_label} table have no "
                f"assigned domain (viral and metagenome/uncultured entries) and so "
        ...
        line += " They can still be pulled by name (e.g. `-t Uroviricota`)."
