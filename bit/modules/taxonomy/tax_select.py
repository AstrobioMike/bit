...
from bit.modules.taxonomy.tax_ranks import (RANKS, NA, REFERENCE_VALUE, accession_core,
...
def resolve_taxon(path, taxon, rank=None, domain=None):
    ...
    (e.g. a name used as both an order and a family). On the NCBI side a user can
    ...
    the bare name would mix domains together.

    `rank` and `domain` are independent disambiguators and may be given together. A
    `domain` is normalized through the same alias table as `-t` (so 'eukarya' ->
...
def diagnose_empty_pool(path, source, rank, taxon, domain=None, reps_only=False,
    ...
    # "it has: complete, scaffold, contig" is only a coherent thing to say when the
    # level filter is the SOLE culprit. With a second culprit alongside it there is no
    # single pool to describe
    present_levels = []
