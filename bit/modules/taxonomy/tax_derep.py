"""
Genome/accession dereplication and minor quality picking from the GTDB / NCBI Parquet assets

MIKE, remember this is not the same dereplication  `gen-mg` uses. That one lives in
modules/gen_mg/selection.py (_select_gtdb_one_per_rank) and does a different job:
seeded-RANDOM sampling toward a target genome count, with group/accession exclusion
for the suppression backfill, and an `off` mode that widens the pool to the full GTDB
table under a fixed quality floor.

This one is DETERMINISTIC best-per-group picking with no target count, shared by
`get-accs-from-gtdb` / `get-accs-from-ncbi`. Keep them separate, they do different jobs.

This is almost a direct port of what i'm using in gtotree for dereplication, minus the
high/low count warnings that are relevant for treeing but not here
"""
...
from bit.modules.taxonomy.tax_select import (
    SOURCES, select, resolve_taxon, live_accession_cores, diagnose_empty_pool)
...
# A RefSeq "reference genome" is preferred over a merely higher-checkm genome (only
# when derep is on), but only if it is not actually bad. These gates are deliberately
# loose, here to catch junk, not to regularly replace "reference" genomes
...
# Per-domain overrides to DEREP_STEPS for the `auto` default. Mostly because euks
...
# selection's target belongs to that domain -- so `-t Eukaryota`, `-t <a eukaryotic
# phylum>`, `-t <a eukaryotic class>` are all coarsened. A name shared across domains
...
from bit.modules.taxonomy.exclusion_list import (filter_rows_by_exclusion,
                                                  exclusion_warning)
    ...
    if derep_rank == "auto":
        ...
        if eff is None:
            warnings.append(
                ...
                f"single best representative, pass --derep-rank {w_rank} explicitly.")
        return eff, warnings
...
def derep(path, source, wanted_rank, wanted_taxon, derep_rank,
    ...
    """
    # Returns (accessions, groups_seen, warnings, attrition), where `attrition` is a
    # PoolAttrition recording how many candidates survived each narrowing stage. It
    # replaces the bare num_excluded this used to return -- that count now lives on
    # the record as `attrition.n_excluded`, alongside the sibling counts an empty
    # selection needs in order to say which stage emptied it.
    warnings = []
...
class RefGenomeSelection:
    ...
    Attributes:
        ...
        attrition    -- the PoolAttrition record for this selection: how many
                        candidates survived each narrowing stage. Always present.
                        Interesting mainly when `accessions` is empty, where it is
                        what lets a caller say WHICH stage emptied it rather than
                        guessing from which flags were passed.
    """
...
def select_ref_genomes(path, source, taxon, target_rank=None, derep_rank="auto",
    ...
    target_domain:
        ...
        domains. Normalized through the same aliases as `-t` ('eukarya' -> 'Eukaryota').
    ...
    diagnose_empty:
        ...
        empty -- diagnosing each one would be pure waste. The `-t` wrappers, which
...
def select_all_domains(path, source, derep_rank="auto", reps_only=None, pick="quality",
    ...
    # Attached rather than warned about here: the CLIs decide the wording.
