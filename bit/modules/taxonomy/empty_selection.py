...
#: `bit-dl-ncbi-assemblies` has `--representatives-only`, `bit-get-accessions-from-gtdb`
#: has `-G/--gtdb-representatives-only` and `-R/--refseq-ref-genomes-only`. So
#: POOL_FILTER_FLAGS["reps_only"] is only a fallback; callers pass `reps_flag` to name
#: their own, and `reps_widen_hint` to name the way back out where one exists.
...
def empty_selection_message(selection, *, taxon_flag="-t", assembly_levels=None,
