"""
The surface `bit get-accs-from-gtdb` and `bit get-accs-from-ncbi` share
"""

from bit.modules.taxonomy.tax_counts import (count_distinct_taxa, count_genomes,
                                               derep_size, rank_counts, read_pool,
                                               representatives_filter)
from bit.modules.taxonomy.tax_derep import (resolve_derep_rank, select_all_domains,
                                              select_ref_genomes)
...
SOURCE_OVERLAP_NOTE = (
    ...
    "derived from a GenBank one), so `--ncbi-section both` will return near-duplicate "
    "assemblies. You most likely only want one of them.")
...
class PoolSpec:
    ...
    def __init__(self, table_path, source, rep_filter=None, accession_prefixes=None,
                 assembly_levels=None, label=None, taxon_flag="-t",
                 exclude_cores=None):
...
def with_filters_implicated_note(text):
    ...
    "No accessions were found for the -t target 'Trichodesmium'." ->
    "No accessions were found for the -t target 'Trichodesmium' with the specified
     filters."
    """
...
def scoped_counts_note(taxon_flag="-t"):
