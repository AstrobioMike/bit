...
"""
The one implementation behind bit's hosted Parquet reference assets.
...
`bit/modules/ncbi/get_ncbi_assembly_data.py` and `bit/modules/gtdb/get_gtdb_data.py`
...
This mirrors gtotree/utils/misc/hosted_parquet_asset.py
"""
...
class HostedParquetAsset:
    ...
    def report_unavailable(self, err):
        ...
        wprint(color_text(f"  Couldn't download the prepared {self.display_name} :(",
                          "yellow"))
        report_message(f"Underlying issue: {err}", color=None,
        ...
        report_message("This is usually a transient network problem, and trying again "
                       ...
                       "fetched manually from:", color=None,
                       initial_indent="    ", subsequent_indent="    ")
        ...
        report_message(f"and placed (as '{self.parquet_filename}' and "
                       ...
                       "`bit data locations check`.", color=None,
                       initial_indent="    ", subsequent_indent="    ")
        print("")
