"""
`--derep-rank` now defaults to 'auto' on both get-accs helpers, matching
dl-ncbi-assemblies and the GToTree main program.
...
from bit.cli.get_accessions_from_gtdb import (build_parser as gtdb_parser,
                                              preflight_checks as gtdb_preflight)
from bit.cli.get_accessions_from_ncbi import (build_parser as ncbi_parser,
                                              preflight_checks as ncbi_preflight)
from bit.modules.taxonomy.get_accs_shared import (DEREP_DEFAULT, DEREP_OFF,
                                                  DEREP_UNSET,
...
def test_explicitly_typed_auto_still_counts_as_explicit():
    ...
    """
    args = argparse.Namespace(derep_rank="auto")
    assert apply_derep_default(args) is True
...
def test_ncbi_helper_defaults_to_auto():
    assert _ncbi(["-t", "Alteromonas"]).derep_rank == "auto"


def test_gtdb_helper_defaults_to_auto():
    args = gtdb_parser().parse_args(["-t", "Archaea"])
    gtdb_preflight(args)
...
def test_ncbi_section_defaults_to_both():
    ...
    to matching dl-ncbi-assemblies.
    """
    assert _ncbi(["-t", "Alteromonas"]).ncbi_section == "both"
...
def test_taxid_quietly_drops_the_inherited_default():
    ...
    assert _ncbi(["-t", "28108"]).derep_rank == "off"


def test_taxid_with_an_explicit_derep_rank_still_errors():
    with pytest.raises(SystemExit):
        _ncbi(["-t", "28108", "--derep-rank", "genus"])
...
def test_taxid_with_explicit_auto_errors_too():
    ...
    """
    with pytest.raises(SystemExit):
        _ncbi(["-t", "28108", "--derep-rank", "auto"])


def test_taxid_with_explicit_off_is_fine():
    assert _ncbi(["-t", "28108", "--derep-rank", "off"]).derep_rank == "off"


def test_a_named_target_keeps_an_explicit_derep_rank():
    assert _ncbi(["-t", "Alteromonas", "--derep-rank", "family"]).derep_rank == "family"
