...
from bit.modules.taxonomy.tax_ranks import RANKS, normalize_taxon_name
from bit.modules.taxonomy.tax_select import (TaxonNotFound, find_ranks_for_taxon,
                                               resolve_taxon)
...
class TestNormalizeTaxonName:
    ...
    def test_unknown_names_pass_through(self):
        """An alias table must never invent a taxon -- only redirect known spellings."""
...
class TestAliasesResolveAgainstTheAsset:
    ...
    def test_an_alias_for_something_absent_still_fails_cleanly(self, tmp_path):
        """
        GTDB has no eukaryotes. `-t eukarya` there must still be a plain
