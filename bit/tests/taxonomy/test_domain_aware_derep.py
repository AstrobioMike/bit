...
`-t all` against the NCBI asset includes Eukaryota, which is far more genome-heavy per rank
...
`-t Eukaryota`, a eukaryotic phylum, and a eukaryotic class are all coarsened.
...
from bit.modules.taxonomy.tax_derep import (
    DEREP_STEPS,
...
################################################################################
# include_rows: skipping metadata must not change WHICH genomes are selected.
#
# And the reason counts still come from the selection path rather than the cheaper
# tax_counts.derep_size(): derep_size counts distinct groups present in the table, so
# it cannot see liveness screening or the quality floor, and over-reports.
################################################################################
...
from bit.modules.taxonomy.tax_counts import derep_size


_GTDB_EXTRA = ["ncbi_refseq_category", "gtdb_representative",
               "checkm2_completeness", "checkm2_contamination",
               "genome_size", "contig_count"]


def _gtdb_rec(acc, lineage, completeness="99.0"):
    d = {"ncbi_genbank_assembly_accession": acc, "ncbi_refseq_category": "na",
         "gtdb_representative": "t", "checkm2_completeness": completeness,
...
def three_families(tmp_path):
    ...
    One phylum, three families of one genome each:
      Fam1 -- fine
      Fam2 -- its only genome is suppressed at NCBI (absent from the screen asset)
      Fam3 -- its only genome is low completeness
    Returns (gtdb_path, ncbi_screen_path).
    """
    recs = [
        _gtdb_rec("GCA_000000010.1", ("Bacteria", "Bphy", "Bcls", "Bord", "Fam1",
                                      "G1", "G1 sp1")),
        _gtdb_rec("GCA_000000020.1", ("Bacteria", "Bphy", "Bcls", "Bord", "Fam2",
                                      "G2", "G2 sp1")),
        _gtdb_rec("GCA_000000030.1", ("Bacteria", "Bphy", "Bcls", "Bord", "Fam3",
                                      "G3", "G3 sp1"), completeness="40.0"),
    ]
    gtdb = tmp_path / "gtdb-data.parquet"
    keys = ["ncbi_genbank_assembly_accession"] + list(RANKS) + _GTDB_EXTRA
    cols = {k: [r[k] for r in recs] for k in keys}
    pq.write_table(
        pa.table({k: pa.array(v, type=pa.string()) for k, v in cols.items()}),
        str(gtdb))

    ncbi = tmp_path / "ncbi-data.parquet"
    pq.write_table(
        pa.table({"assembly_accession": pa.array(
            ["GCA_000000010.1", "GCA_000000030.1"], type=pa.string())}), str(ncbi))

    return str(gtdb), str(ncbi)
...
class TestIncludeRowsDoesNotChangeSelection:
    ...
    def test_accessions_are_identical_with_and_without_rows(self, three_families, derep):
        gtdb, _ncbi = three_families
        with_rows = select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank=derep,
                                       reps_only=False, include_rows=True)
        without = select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank=derep,
                                     reps_only=False, include_rows=False)
        assert with_rows.accessions == without.accessions

    def test_rows_are_carried_only_when_asked_for(self, three_families):
        gtdb, _ncbi = three_families
        assert select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank="family",
                                  reps_only=False, include_rows=True).rows
        assert select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank="family",
                                  reps_only=False, include_rows=False).rows == []

    def test_warnings_and_resolution_survive_without_rows(self, three_families):
        """The reporting layer reads these, so dropping rows must not drop them."""
        gtdb, ncbi = three_families
        sel = select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank="family",
                                 reps_only=False, screen_against=ncbi,
                                 include_rows=False)
...
class TestCheapCountPathWouldOverreport:
    ...
    table. It never applies `screen_against` or the quality floor, so groups whose
    only genome is suppressed or below the floor still get counted. A dry run built
    on it would promise more genomes than the real run downloads.
    """

    def test_derep_size_counts_every_group_in_the_table(self, three_families):
        gtdb, _ncbi = three_families
        assert derep_size(gtdb, "gtdb", "phylum", "Bphy", "family") == 3
    ...
    def test_a_quality_floor_makes_it_smaller_still(self, three_families):
        ...
        sel = select_ref_genomes(gtdb, "gtdb", "Bphy", derep_rank="family",
                                 reps_only=False, screen_against=ncbi,
                                 min_completeness=90.0, include_rows=False)
