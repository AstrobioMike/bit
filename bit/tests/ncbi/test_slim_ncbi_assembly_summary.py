...
def _write_summary(path, accessions, gzip_it=False, reorder=False):
    ...
    if reorder:
        # move ftp_path and taxid far from their usual spots
        import random
...
def test_build_slim_keeps_only_kept_columns_in_order(tmp_path):
    ...
    # every data row has exactly len(KEPT_COLUMNS) fields
    for line in lines[1:]:
...
def test_build_slim_values_land_in_right_columns(tmp_path):
    ...
    # a sentinel column that should NOT be present
...
def test_write_date_retrieved_format(tmp_path):
    ...
    from bit.modules.ncbi.slim_ncbi_assembly_summary import write_date_retrieved
    p = tmp_path / "date-retrieved.txt"
    write_date_retrieved(p, when=date(2026, 4, 1))
    assert p.read_text() == "2026,04,01\n"


def test_write_date_retrieved_roundtrips_into_gen_metagenome_reader(tmp_path):
    """The stamped date must parse cleanly in gen_metagenome._read_retrieved_date
    (the consumer), not just match a regex."""
    from datetime import date
    from bit.modules.ncbi.slim_ncbi_assembly_summary import write_date_retrieved
    from bit.modules.gen_metagenome import _read_retrieved_date
    write_date_retrieved(tmp_path / "date-retrieved.txt", when=date(2026, 4, 1))
    assert _read_retrieved_date(str(tmp_path)) == "Apr 01, 2026"
